/**
 * Fix Amravati Region URLs with Exact Spacing
 * 
 * This script focuses on fixing dashboard URLs for the Amravati region.
 */
...
function generateCorrectDashboardUrl(scheme) {
  ...
  // Always use "Amaravati" in the URL for Amravati region
  const regionDisplay = "Amaravati";
...
/**
 * Process all schemes in the Amravati region
 */
async function fixAmravatiUrls() {
  ...
  try {
    ...
    try {
      console.log('Processing Amravati region...');
      
      // Get all schemes for the Amravati region
      const schemesResult = await client.query('SELECT * FROM scheme_status WHERE region = $1', ['Amravati']);
      const schemes = schemesResult.rows;
      
      console.log(`Found ${schemes.length} schemes in Amravati region`);
      ...
      console.log(`Completed Amravati region. Updated: ${updatedCount}, Skipped: ${skippedCount}`);
