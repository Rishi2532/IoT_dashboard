/**
 * Fix Dashboard URLs for Amravati Region
 * 
 * This script fixes dashboard URLs for the Amravati region to match the exact format:
 * - Uses display ID 10108
 * - Preserves space between scheme_id and hyphen
 * - Special handling: Changes "Amravati" to "Amaravati" in the URL
...
function generateCorrectDashboardUrl(scheme) {
  ...
  // Special handling: Change "Amravati" to "Amaravati" in the URL
  const regionDisplay = "Amaravati";  // Always use "Amaravati" for this region
...
/**
 * Fix dashboard URLs for the Amravati region
 */
async function fixAmravatiDashboardUrls() {
  ...
  try {
    ...
    try {
      ...
      console.log(`\nCompleted region ${region}. Updated: ${updatedCount}, Skipped: ${skippedCount}`);
...
fixAmravatiDashboardUrls().catch(err => {
