/**
 * Fix Dashboard URLs for Batch 2 (Nagpur, Amravati, Chhatrapati Sambhajinagar)
 * 
 * This script fixes dashboard URLs for the second batch of regions to match the exact format:
 ...
 * - Removes any UUID parts at the end of URLs
 */
...
/**
 * Fix dashboard URLs for the second batch of regions (Nagpur, Amravati, Chhatrapati Sambhajinagar)
 */
async function fixBatch2DashboardUrls() {
  ...
  try {
    ...
    try {
      // Define regions for Batch 2
      const regions = ['Nagpur', 'Amravati', 'Chhatrapati Sambhajinagar'];
      
      console.log(`Processing Batch 2: ${regions.join(', ')}`);
      ...
      for (const region of regions) {
        ...
        for (const scheme of schemes) {
          ...
          if (dashboardUrl) {
            ...
            if (updatedCount % 5 === 0) {
              console.log(`Updated ${updatedCount} schemes so far in region ${region}`);
            }
          } else {
      ...
      console.log('\nBatch 2 dashboard URLs have been updated to the correct format.');
...
fixBatch2DashboardUrls().catch(err => {
