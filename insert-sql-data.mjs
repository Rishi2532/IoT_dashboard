...
async function executeSqlInsert() {
  ...
  try {
    ...
    // Extract scheme_ids from the SQL file for deletion
    const sqlFilePath = path.join(__dirname, 'attached_assets', 'insert_scheme_status.sql');
    const sql = fs.readFileSync(sqlFilePath, 'utf8');
    
    // Extract scheme_ids from the SQL
    const schemeIdRegex = /'(\d+)'/g;
    const schemeIds = [];
    let match;
    while ((match = schemeIdRegex.exec(sql)) !== null) {
      schemeIds.push(match[1]);
    }
