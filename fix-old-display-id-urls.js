/**
 * Fix Old Display ID URLs Script
 * 
 * This script specifically targets and fixes schemes with the old display ID (10108) in their dashboard URLs
 */
...
/**
 * Main function to fix old display ID dashboard URLs
 */
async function fixOldDisplayIdUrls() {
  ...
  try {
    ...
    try {
      // Get count of schemes with old display ID
      const countResult = await client.query("SELECT COUNT(*) FROM scheme_status WHERE dashboard_url LIKE '%/Displays/10108/%'");
      const totalOldUrls = parseInt(countResult.rows[0].count);
      
      if (totalOldUrls === 0) {
        console.log('No schemes with old display ID (10108) found!');
        return;
      }
      
      console.log(`Found ${totalOldUrls} schemes with old display ID (10108) to update.`);
      
      // Get all schemes with old display ID
      const result = await client.query("SELECT * FROM scheme_status WHERE dashboard_url LIKE '%/Displays/10108/%'");
      ...
      for (const scheme of schemes) {
        ...
        if (dashboardUrl) {
          ...
          updatedCount++;
          console.log(`Updated ${updatedCount} of ${totalOldUrls}: ${scheme.scheme_name} (${scheme.scheme_id}) in ${scheme.block}`);
        } else {
          skippedCount++;
      ...
      console.log(`- Total schemes with old display ID: ${totalOldUrls}`);
