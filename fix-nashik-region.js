/**
 * Fix URLs for Nashik Region
 * 
 * This script updates dashboard URLs for the Nashik region to match
...
/**
 * Process Nashik region only
 */
async function fixNashikRegion() {
  ...
  try {
    ...
    try {
      console.log('\nProcessing region: Nashik');
      
      // Get all schemes for Nashik region
      const schemesResult = await client.query('SELECT * FROM scheme_status WHERE region = $1', ['Nashik']);
      const schemes = schemesResult.rows;
      
      console.log(`Found ${schemes.length} schemes in region Nashik`);
      ...
      console.log(`Completed region Nashik. Updated: ${updatedCount}, Skipped: ${skippedCount}`);
      console.log('\nNashik region has been updated.');
