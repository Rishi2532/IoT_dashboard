/**
 * Fix URLs for Amravati Region
 * 
 * This script updates dashboard URLs for the Amravati region to match
 * the format used by non-Pune regions
 */
...
function generateFiveRegionUrl(scheme) {
  ...
  let regionName = 'Amaravati'; // Always use Amaravati in the URL for Amravati region
...
/**
 * Process Amravati region only
 */
async function fixAmravatiRegion() {
  ...
  try {
    ...
    try {
      console.log('\nProcessing region: Amravati');
      
      // Get all schemes for Amravati region
      const schemesResult = await client.query('SELECT * FROM scheme_status WHERE region = $1', ['Amravati']);
      const schemes = schemesResult.rows;
      
      console.log(`Found ${schemes.length} schemes in region Amravati`);
      ...
      console.log(`Completed region Amravati. Updated: ${updatedCount}, Skipped: ${skippedCount}`);
      console.log('\nAmravati region has been updated.');
