 ...
 * 
 * This script checks for schemes and villages with missing dashboard URLs
 * and regenerates them using the proper format. This is especially important
 * when running the application locally in VSCode where dashboard buttons may not
...
async function fixMissingDashboardUrls() {
  ...
  try {
    ...
    const schemeResults = await client.query({
      text: 'SELECT * FROM scheme_status WHERE dashboard_url IS NULL OR dashboard_url = \'\';'
    });
    ...
    if (schemeResults.rows.length > 0) {
      ...
      for (const scheme of schemeResults.rows) {
        ...
        if (dashboardUrl) {
          ...
          await client.query({
            text: 'UPDATE scheme_status SET dashboard_url = $1 WHERE scheme_id = $2 AND scheme_name = $3',
            values: [dashboardUrl, scheme.scheme_id, scheme.scheme_name]
          });
        }
      }
      
      console.log('Scheme dashboard URLs fixed successfully!');
    } else {
    ...
    const villageResults = await client.query({
      text: 'SELECT * FROM water_scheme_data WHERE dashboard_url IS NULL OR dashboard_url = \'\';'
    });
    ...
    if (villageResults.rows.length > 0) {
      ...
      for (const village of villageResults.rows) {
        ...
        if (dashboardUrl) {
          ...
          await client.query({
            text: 'UPDATE water_scheme_data SET dashboard_url = $1 WHERE scheme_id = $2 AND village_name = $3',
            values: [dashboardUrl, village.scheme_id, village.village_name]
          });
        }
      }
      
      console.log('Village dashboard URLs fixed successfully!');
    } else {
      console.log('No villages with missing dashboard URLs found.');
    }
    
    client.release();
    console.log('All dashboard URLs have been fixed!');
  } catch (error) {
    console.error('Error fixing dashboard URLs:', error);
  } finally {
    await pool.end();
  }
}
...
// Run the function
fixMissingDashboardUrls().catch(console.error);
