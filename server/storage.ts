...
export class PostgresStorage implements IStorage {
  ...
  async getChlorineSensorsWithWater(regionName?: string): Promise<{
    ...
    try {
      ...
      console.log(`Found ${result.rows ? result.rows.length : 0} chlorine sensors with water`);
  ...
  async getPressureSensorsWithWater(regionName?: string): Promise<{
    ...
    try {
      ...
      console.log(`Found ${result.rows ? result.rows.length : 0} pressure sensors with water`);
  ...
  async getChlorineDashboardStats(regionName?: string): Promise<{
    ...
    try {
      ...
      console.log(
        "Consistent below range sensors:",
      ...
      console.log(
        "Consistent optimal range sensors:",
      ...
      console.log(
        "Consistent above range sensors:",
      ...
      if (filter) {
        ...
        if (filter.pressureRange) {
          ...
          if (filter.minPressure !== undefined) {
            query = query.where(
              sql`${pressureData.pressure_value_7} >= ${filter.minPressure}`,
            );
          }

          if (filter.maxPressure !== undefined) {
            query = query.where(
              sql`${pressureData.pressure_value_7} <= ${filter.maxPressure}`,
            );
          }
        }
      }
  ...
  async createScheme(scheme: InsertSchemeStatus): Promise<SchemeStatus> {
    ...
    const result = await db
      .insert(schemeStatuses)
      .values(schemeWithUrl)
      .returning();
    return result[0];
  }
  ...
  private generateSpecialCaseVillageUrl(
    ...
    if (
      ...
      // Base URL parameters
      const BASE_URL =
        "https://14.99.99.166:18099/PIVision/#/Displays/10109/CEREBULB_JJM_MAHARASHTRA_VILLAGE_LEVEL_DASHBOARD";
      const STANDARD_PARAMS = "hidetoolbar=true&hidesidebar=true&mode=kiosk";
  ...
  private generateSpecialCaseEsrUrl(
    ...
    if (
      ...
      const BASE_URL =
        "https://14.99.99.166:18099/PIVision/#/Displays/10086/CEREBULB_JJM_MAHARASHTRA_ESR_LEVEL_DASHBOARD";
  ...
  private generateVillageDashboardUrl(village: WaterSchemeData): string | null {
    ...
    // Handle the special case for Amravati region (change to Amaravati in the URL)
    const regionDisplay =
      village.region === "Amravati" ? "Amaravati" : village.region;
