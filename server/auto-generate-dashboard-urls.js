...
async function autoGenerateDashboardUrls() {
  ...
  try {
    ...
    // Check if tables exist before trying to update URLs
    ...
    const schemeTableExists = await checkTableExists('scheme_status');
    if (!schemeTableExists) {
      console.log('⚠️ scheme_status table does not exist yet. Skipping URL generation.');
    } else {
    ...
    // Check if water_scheme_data table exists
    const villageTableExists = await checkTableExists('water_scheme_data');
    if (!villageTableExists) {
      console.log('⚠️ water_scheme_data table does not exist yet. Skipping URL generation.');
    } else {
    ...
    console.log('✅ Dashboard URL generation check completed!');
  } catch (error) {
