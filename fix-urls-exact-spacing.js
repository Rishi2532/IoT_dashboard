/**
 * Fix Dashboard URLs with Exact Spacing
 * 
 * This script fixes dashboard URLs for all schemes to match the exact format:
 * - Uses display ID 10108
 * - Uses specific space format:
 *   * "Block -Mohol" (space before the hyphen)
 *   * "Scheme - 20029637 -Penur Patkul" (space before and after first hyphen, 
 *     no space between second hyphen and scheme name)
...
async function fixAllRegionsUrls() {
  ...
  try {
    ...
    try {
      ...
      console.log('\nAll dashboard URLs have been updated with the exact spacing format.');
