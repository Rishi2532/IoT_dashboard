/**
 * Fix Dashboard URLs with UUID For All Schemes
 * 
 ...
 * - Includes the UUID at the end after "?"
 */
...
// UUID to append to all dashboard URLs
const SCHEME_UUID = "cc69f6d6-feff-11ef-b0d9-18c04d3c6ae7";

/**
 * Generate a dashboard URL with the correct format including UUID
...
function generateCorrectDashboardUrl(scheme) {
  ...
  // Create the path using the correct format with a space between scheme_id and hyphen, and UUID
  const path = `\\\\DemoAF\\JJM\\JJM\\Maharashtra\\Region-${regionDisplay}\\Circle-${scheme.circle}\\Division-${scheme.division}\\Sub Division-${scheme.sub_division}\\Block-${scheme.block}\\Scheme-${scheme.scheme_id} - ${scheme.scheme_name}?${SCHEME_UUID}`;
...
async function fixAllRegionsUrls() {
  ...
  try {
    ...
    try {
      ...
      console.log('\nAll dashboard URLs have been updated to the correct format with UUID.');
