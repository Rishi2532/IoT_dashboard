...
const LpcdImport = () => {
  ...
  const amravatiMutation = useMutation({
    mutationFn: async () => {
      ...
      if (!response.ok) {
        const error = await response.json();
        throw new Error(error.message || 'Failed to import Amravati format file');
      }
  ...
  const downloadTemplate = () => {
    ...
    const sampleData = "Region,Circle,Division,Sub-Division,Block,Scheme ID,Scheme Name,Village Name,Population,Number of ESR,Water Value Day 1-7,LPCD Value Day 1-7,Water Date Day 1-6,LPCD Date Day 1-7\n" +
              ...
              <CardContent>
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="text-sm text-gray-600 mb-4 pl-5">
                      <p>Upload Excel files with the Amravati-specific format.</p>
                      <p>This format is specifically designed for the Amravati region's data structure.</p>
                    </div>
                    ...
                    <div className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:bg-gray-50 transition-colors">
                      ...
                      <label htmlFor="amravatiFile" className="cursor-pointer">
                        <div className="flex flex-col items-center">
                          <UploadCloud className="h-10 w-10 text-gray-400 mb-2" />
                          <p className="text-sm font-medium">Click to select an Amravati format file</p>
                          <p className="text-xs text-gray-500">
                            or drag and drop it here
                          </p>
                          {amravatiFile && (
                            <div className="mt-2 p-2 bg-blue-50 rounded-md text-left w-full">
                              <p className="text-sm font-medium text-blue-700">{amravatiFile.name}</p>
                              <p className="text-xs text-gray-500">
                  ...
                  {amravatiMutation.isSuccess && (
                    <Alert className="mt-4 bg-green-50 border-green-200">
                      ...
                      <AlertDescription>
                        ...
                        <ul className="list-disc list-inside mt-1 text-sm">
                          <li>Inserted records: {amravatiMutation.data?.inserted || 0}</li>
                          <li>Updated records: {amravatiMutation.data?.updated || 0}</li>
                          <li>Total processed: {(amravatiMutation.data?.inserted || 0) + (amravatiMutation.data?.updated || 0)}</li>
                        </ul>
                        ...
                        {amravatiMutation.data?.errors && amravatiMutation.data.errors.length > 0 && (
                          <div className="mt-2">
                            <p className="font-medium">Some records had errors:</p>
                            <div className="max-h-40 overflow-y-auto mt-1 p-2 bg-red-50 rounded text-sm">
                              {amravatiMutation.data.errors.map((error, idx) => (
                                <p key={idx} className="text-red-700">{error}</p>
                              ))}
                            </div>
                          </div>
                  ...
                  {amravatiMutation.isError && (
                    <Alert variant="destructive" className="mt-4">
                      ...
                      <AlertTitle>Import failed</AlertTitle>
                      <AlertDescription>
                        {amravatiMutation.error instanceof Error ? amravatiMutation.error.message : 'Failed to import file'}
                      </AlertDescription>
                    </Alert>
                  )}
                </div>
