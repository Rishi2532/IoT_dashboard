/**
 * Fix URLs for Konkan Region
 * 
 * This script updates dashboard URLs for the Konkan region to match
...
/**
 * Process Konkan region only
 */
async function fixKonkanRegion() {
  ...
  try {
    ...
    try {
      console.log('\nProcessing region: Konkan');
      
      // Get all schemes for Konkan region
      const schemesResult = await client.query('SELECT * FROM scheme_status WHERE region = $1', ['Konkan']);
      const schemes = schemesResult.rows;
      
      console.log(`Found ${schemes.length} schemes in region Konkan`);
      ...
      console.log(`Completed region Konkan. Updated: ${updatedCount}, Skipped: ${skippedCount}`);
      console.log('\nKonkan region has been updated.');
