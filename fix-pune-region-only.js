/**
 * Fix URLs for Pune Region Only
 * 
 * This script updates dashboard URLs for the Pune region to match its unique format:
 * - Block -Mohol (space before hyphen)
 * - Scheme - 20029637 -Penur Patkul (spaces before and after first hyphen, no space after second)
...
/**
 * Generate a dashboard URL with the format used by the Pune region
 ...
 */
function generatePuneRegionUrl(scheme) {
  ...
  // Create the path using the Pune format (space before hyphen in Block, spaces around scheme ID)
...
/**
 * Process Pune region only
 */
async function fixPuneRegionOnly() {
  ...
  try {
    ...
    try {
      ...
      for (const scheme of schemes) {
        ...
        const dashboardUrl = generatePuneRegionUrl(scheme);
